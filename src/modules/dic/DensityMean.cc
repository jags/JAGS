...
namespace dic {
		...
		_n(0), _dim(dim)
    {
		...
		if ( monitor_name.compare(0, cdt.length(), cdt) != 0 ) {
			throw std::logic_error("Incorrect density type reported in monitor_name for DensityMean");
		}
		
		if ( monitor_name.find("_mean") == string::npos) {
			throw std::logic_error("Incorrect monitor type reported in monitor_name for DensityMean");
		}
    }
