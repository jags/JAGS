...
\verb+}+ and preceded by the keyword \verb+model+. Here is a simple
...
in the model (together with the parent/child relationships represented
as directed edges) form a directed acyclic graph. The very top-level
...
in the data file ({\em e.g.}  \verb+x[1]+).
...
with the data to create a {\em virtual graphical model}, a
...
\JAGS\ compiler will stop with an error message (See below).
...
\end{verbatim}
This model may be compiled by combining it with the following data.
\begin{verbatim}
x <- c(1, 2, 3, 4, 5)
Y <- c(1, 3, 3, 3, 5)
N <- 5
\end{verbatim}
In this model, data values must always be supplied for \texttt{N},
since it is used to index the for loop, and \texttt{x} since it is
used on the right hand side of the relation defining \texttt{mu} but
never defined on the left hand side of a relation.
...
As with the data, initial values are supplied for whole node arrays
and you may include missing values in the initial values.  This need
typically arises with contrast parameters.  Suppose $X$ is a
categorical covariate, taking values from 1 to 4, which is used a a
predictor variable in a linear model:
\begin{verbatim}
for (i in 1:N) {
   Y[i] ~ alpha + beta[x[i]]
}
...
all chains. This is undesirable behaviour and it will be changed in a
...
are avaiable: monitors of type ``mean'' and ``variance'' defined by
...
is. Please send a bug report (see ``Feedback'' on
...
have been developed that offer a simplified interface to \JAGS. The
main ones are detailed below. There are many other R packages that
...
To get samples from the posterior distribution of the parameteres, we
...
some novel distributions.

The basic function for running a model with the \texttt{runjags}
package is the \texttt{run.jags()} function:
\begin{verbatim}
library(runjags)
out <- run.jags(model="line.bug", monitor=c("alpha","beta","sigma"),
...
alpha   1.9761  2.9997  4.0406  3.002  0.5363   -- 0.0037448     0.7 20509
beta  0.065332 0.79686   1.503 0.8001 0.39573   -- 0.0027221     0.7 21135
sigma  0.32386 0.82087  2.1403 1.0062 0.72386   -- 0.0094204     1.3  5904
                       
           AC.10   psrf
alpha -0.0052736 1.0001
beta    -0.00539  1.001
sigma -0.0034472 1.0001
...
The plot method for \texttt{runjags} objects shows: a traceplot, the
...
for all variables is also generated.
