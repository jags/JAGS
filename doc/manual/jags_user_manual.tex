...
\texttt{bugs} module that take general arguments.
...
\hline
\verb+inprod(x1,x2)+ & Inner product \\
...
\subsection{Linear interpolation functions}
...
Function & Description \\
\hline
\verb+interp.lin(p, gx, v)+    & Linear Interpolation \\
\verb+interp.lin2d(p, gx, gy, v)+ & Bilinear Interpolation \\
\verb+interp.lin3d(p, gx, gy, gz, v)+ & Trilinear Interpolation \\
\hline
\end{tabular}
\caption{Linear interpolation functions in the \texttt{bugs} module \label{table:bugs:interplin}}
\end{table}

\JAGS\ can carry out function approximation by linear interpolation in
up to 3 dimensions using the \texttt{interp.lin*} functions shown in
table~\ref{table:bugs:interplin}. The arguments are:
\begin{itemize}
\item A point in space $p$ at which the function is to be
  approximated. For \texttt{interp.lin} $p$ is a scalar. For
  \texttt{interp.lin2d} and \texttt{interp.lin3d} it is a vector of
  length 2 and 3 respectively.
\item A sequence of up to 3 grid vectors $gx,gy,gz$, one for each
  dimension, showing the coordinates at which the target function has
  been evaluated. All grid vectors must be strictly increasing and of
  length at least 2.
\item An array $v$ giving the values of the target function evaluated
  on a grid defined by the outer product of the grid vectors. For
  \texttt{interp.lin}, $v$ is a vector, for \texttt{interp.lin2d} it
  is a matrix, and for \texttt{interp.lin3d} it is a 3-dimensional
  array.
\end{itemize}
If a point $p$ is outside of the grid in any dimension then it takes
the value of nearest boundary point.  For example, the output of
\texttt{interp.lin} is constant below the lowest grid value and above
