...
\end{verbatim}
The resulting object \texttt{model} is of class ``jags´´. It does not
contain any samples from the posterior. To get parameter samples, we use
the \texttt{coda.samples} function after first using the \texttt{update} function
to run the Markov Chain for a burn-in period of 1000 iterations.
\begin{verbatim}
update(model, n.iter=1000) 
samples <- coda.samples(model, c("alpha", "beta", "sigma"), n.iter=1000)
\end{verbatim}
The \texttt{samples} object is of class \texttt{mcmc.list} defined by
the \texttt{coda} package \citep{coda}. It contains monitored samples
  of the variables \texttt{alpha}, \texttt{beta}, and \texttt{sigma}
  for the two parallel chains.

To get a readable summary of the samples, we use the \texttt{summary}
function:
\begin{verbatim}
> summary(samples)
...
\begin{verbatim}
out <- autojags(data=line_data, inits=line_inits,
