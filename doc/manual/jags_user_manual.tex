...
\caption{Scalar-valued functions with general
  ...
  Elipses $\ldots$ mean that the function takes a variable number of
  arguments}
\end{table}
...
\verb+v[2]+, \verb+v[2]+, $\ldots$ \verb+v[M]+, \verb+v[M]+, $\ldots$.
...
with the smallest index in the input vector. The \texttt{rank}
...
the one in R. The latter uses a similar parameterization to the
...
\]
With the \verb{dgen.gamma} distribution $\lambda$ is not a hazard
...
y ~ dbern(p)
\end{verbatim}
for $0 < p < 1$. The \verb{dbern} distribution has probability mass function
\begin{eqnarray*}
  P(Y=1) & = & p \\
  P(Y=0) & = & 1 - p
\]
...
y ~ dcat(pi)
\end{verbatim}
where $\pi$ (pi) is a vector of non-negative probability weights of
...
%initial values need to be consistent
%or you could fix the censored sampler
...
can be negative, {\em e.g.} if $d=-2$ then the data are rounded to the
nearest $100$.
...
There is a small amount of tolerence for ``near equality´´
is allowed in this comparison to allow for the fact that, in general,
decimal fractions cannot represented exactly.

\subsubsection{Summing: \texttt{sum}}

The \texttt{sum} function (See table \label{table:bugs:scalar2}) is
a scalar-valued function that return the sum of all of its arguments.
\begin{verbatim}
y <- sum(x1, x2, x3)
\end{verbatim}
...
example can be seen in section \ref{bugs:dhyper}, where the
...
them together ensuring that the constraint that $y$ is the sum of the
arguments $x1, x2, \ldots$ is preserved. Furthermore, if one of the
arguments is itself the sum of unobserved stochastic nodes, then the
\texttt{bugs::Sum} sampler will update them together. For example, if
\begin{verbatim}
y ~ dsum(x1, z)
z <- x2 + x3
\end{verbatim}
...
are censored by the \texttt{dinterval} distribution (See section
\ref{bugs:dinterval}). If the node has a distribution that can be
bounded then the \texttt{bugs::Censored} sampler draws samples from
the truncated distribution.

\subsubsection{\textt{bugs::Sum}}

The \texttt{bugs::Sum} factory creates samplers for nodes that are
constrained by the \textttt{sum} observable function. See section
\ref{bugs:sum} for more details.
...
as a set of normalized gamma distributions. (See section
\ref{bugs:ddirch})
...
ordered logistic regression model \cite{McCullagh1980}. For example,
suppose
\[
\mu = \alpha + \beta x
\]
...
logistic distribution (section \ref{glm:dordered.logit}) except that
...
\caption{The half-t distribution with scale $S$ and 2 degrees of
  ...
  degrees of freedom increases to infinity}
...
sparse matrix libraries \cite{Davis2006, Davis1999} which allows
