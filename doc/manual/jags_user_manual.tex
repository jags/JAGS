...
The \texttt{bugs::Finite} factory produces the \texttt{bugs::Finite}
sampler for discrete-valued, bounded stochastic nodes ({\em i.e.}
those that can take only a finite number of values). The
\texttt{bugs::Finite} sampler draws samples by inverting the
...
\texttt{bugs::Finite} factory will not produce a sampler when there
...
The chi-square distribution on $k > 0$ degrees of freedom is defined by
\begin{verbatim}
y ~ dchisqr(k)
\end{verbatim}
...
The F distribution on $n >0$ and $m > 0$ degrees of freedom is defined by
\begin{verbatim}
y ~ df(n, m)
\end{verbatim}
...
The $F$ distribution can be constructed as the ratio of two chi-square
...
x2 ~ dchisqr(m)
\end{verbatim}
The F distribution has mean $m/(m-2)$ for $m > 2$. The variance is
finite for $m > 4$.
...
with rate $\lambda$. The chi-square distribution is also a special case
...
\end{verbatim}
for $r > 0, b > 0, \lambda > 0$.

For compatibility with OpenBUGS, the alias \texttt{dggamma} may also
be used as the name of the generalized gamma distribution.
...
The non-central chi-square distribution on $k$ degrees of freedom with
...
The non-central chi-square distribution is equivalent to a Poisson
...
y ~ dweib(v, lambda)
\end{verbatim}
for $v > 0, \lambda > 0$. Its probability density function is
\[
...
The Bernoulli distribution (named after Swiss mathematician Jacobe
...
y ~ dbin(p,n)
\end{verbatim}
for $0 < p < 1$ and $n \geq 1$. It has probability mass function
\[
...
outcome $\pi$ satisfies $\pi_i \geq 0$ and $\sum_i \pi_i = 1$ (i.e. the
...
to zero ({\em i.e.}  $\pi_j = 0$ if $\alpha_j = 0$) At least one
...
JAGS recongizes it as the conjugate prior.
...
some time on the interpretation of he parameters $R$ and $k$ in this
...
matrix $\Sigma$. When $k=m+1$ the correlation parameters
\[
