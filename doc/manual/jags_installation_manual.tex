...
are for those users that want to install \JAGS\ from source.
...
needing to build against the libstdc++ library should see section 
\ref{section:otherosxoptions}.  Users with a version of Mac OS X predating 
10.9 (Mavericks) should refer to the installation instructions given in 
older versions of the \JAGS~manual.
...
contained within the macOS installation.  The clang warning
...
\subsection{Other installation options for macOS}
...
Mac OS X 10.6 - 10.9.  This is compatible with the ``Snow Leopard builds'' of
\R\ that have previously been provided on CRAN, and the corresponding 
modules in the \texttt{rjags} and \texttt{runjags} packages. 
...
Apple supplies a compiler suite, BLAS and LAPACK along with the Xcode developer tools, but some
...
This command will take a long time to complete.  The \texttt{-framework Accelerate} flag can
